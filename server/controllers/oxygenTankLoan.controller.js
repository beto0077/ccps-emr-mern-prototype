...
export const getOxygenTankLoans = async (req, res) => {
    try {
        const [result] = await db.query("SELECT * FROM OxygenTankLoanInformation ORDER BY oxygen_loan_id ASC");
        res.json(result);
    } catch (error) {
...
export const getOxygenTankLoan = async (req, res) => {
    try {
        const [result] = await db.query("SELECT * FROM OxygenTankLoanInformation WHERE oxygen_loan_id = ?", [req.params.id]);
        if (result.length === 0) {
...
export const deleteOxygenTankLoan = async (req, res) => {
    try {
        const [result] = await db.query("DELETE FROM OxygenTankLoanInformation WHERE oxygen_loan_id = ?", [req.params.id]);
