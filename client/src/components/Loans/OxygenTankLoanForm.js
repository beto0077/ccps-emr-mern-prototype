...
function OxygenTankLoanForm() {
  ...
  const { createOxygenTankLoan, getOxygenTankLoan, updateOxygenTankLoan } =
    useOxygenTankLoanContext();
  ...
  useEffect(() => {
    ...
    const loadOxygenTankLoan = async () => {
      if (params.id) {
        const loadedLoan = await getOxygenTankLoan(params.id);

        if (
          !loadedLoan.loan_completed &&
          (loadedLoan.return_date === null || loadedLoan.return_date === "NULL")
        ) {
          loadedLoan.return_date = formatDate(new Date());
        }
        ...
        const patient = await getPatient(loan.patient_id);
  ...
  const handleSubmit = async (e) => {
    ...
    if (params.id) {
      await updateOxygenTankLoan(params.id, loanData);
    } else {
      await createOxygenTankLoan(loanData);
    }
    navigate(`/oxygenTankLoanList/${loan.patient_id}`);
    setLoan({
      patient_id: "",
      delivery_date: "",
      return_date: "",
  ...
  return (
    <>
      ...
      >
        <h1>Prestamo de tanque de oxígeno</h1>
...
export default OxygenTankLoanForm;
